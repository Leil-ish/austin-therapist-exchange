...
export async function reviewJoinRequest(formData: FormData) {
  ...
  if (!existingProfile) {
    ...
    await admin.from("profiles").insert({
      ...
      membership_tier: grantPremium ? "premium" : "free",
  ...
  if (!existingTherapistProfile) {
    await admin.from("therapist_profiles").insert({ profile_id: authUserId, ...therapistData });
  } else {
    await admin.from("therapist_profiles").update(therapistData).eq("profile_id", authUserId);
  }
